...
with st.sidebar:
    ...
    loan_amount_existing = st.number_input("Số tiền đã được cấp tín dụng (VND)", min_value=0, value=200000000, step=10000000, key="loan_input")
    st.caption(f"Đã cấp: **{loan_amount_existing:,.0f}** VND")
    ...
    uploaded_file = st.file_uploader("Upload sao kê giao dịch (.xlsx)", type=["xlsx"], key="file_uploader")
        ...
        if 'Noi_Dung' in df.columns and 'So_Tien' in df.columns:
            ...
            if dong_tien_thuan > 0:
                ...
                if final_limit > 0:
                    ...
                    # --- PHẦN TẠO PDF (Đã xử lý lỗi font bằng cách ép kiểu dữ liệu) ---
                    try:
                        pdf = FPDF()
                        pdf.add_page()
                        ...
                        # Loại bỏ dấu tiếng Việt để tránh lỗi latin-1 codec
                        import unicodedata
                        def remove_accents(input_str):
                            nfkd_form = unicodedata.normalize('NFKD', input_str)
                            return "".join([c for c in nfkd_form if not unicodedata.combining(c)])

                        safe_name = remove_accents(customer_name)
                        
                        pdf.cell(200, 10, txt=f"Khach hang: {safe_name}", ln=True)
                        ...
                        st.download_button(
                            ...
                            data=pdf_data,
                            file_name=f"Bao_cao_phe_duyet_{safe_name}.pdf",
                            mime="application/pdf"
                        )
                    except Exception as pdf_err:
                        st.warning(f"Lưu ý: Không thể tạo bản PDF do lỗi font: {pdf_err}")
                else:
